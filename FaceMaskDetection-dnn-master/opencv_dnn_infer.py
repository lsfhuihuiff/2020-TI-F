...
def inference(net, image, conf_thresh=0.5, iou_thresh=0.4, target_shape=(160, 160), draw_result=True, chinese=True):
    ...
    for idx in keep_idxs:
        ...
        if class_id == 1:
            os.system('omxplayer -o local /home/pi/Desktop/face.mp3')
    return image

def run_on_video(Net, conf_thresh=0.5):
    size = (640, 480)
    j = 0
    with PiRGBArray(camera, size) as stream:
        for frame in camera.capture_continuous(stream, format='bgr', splitter_port=2, resize=size,
                                               use_video_port=True):
            img_raw = frame.array  
...
if __name__ == "__main__":
    ...
    parser.add_argument('--proto', type=str, default='/home/pi/Desktop/FaceMaskDetection-dnn-master/models/face_mask_detection.prototxt', help='prototxt path')
    parser.add_argument('--model', type=str, default='/home/pi/Desktop/FaceMaskDetection-dnn-master/models/face_mask_detection.caffemodel', help='model path')
    ...
    run_on_video(Net, conf_thresh=0.5)
    cv2.destroyAllWindows()
    camera.close()
